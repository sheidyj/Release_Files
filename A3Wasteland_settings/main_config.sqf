...
A3W_timeMultiplierDay = 2.3;       // Sets the speed of time between 5 AM and 8 PM (for example, 6.0 means 6 hours in-game will pass in 1 real hour)
...
A3W_spawnBeaconCooldown = 30;    // Number of seconds to wait between each use of a specific spawn beacon (0 = disabled)
...
A3W_atmTransferFee = 1;            // Fee in percent charged to players for money transfers to other players (0 to 50)
...
A3W_vehicleLifetime = 90*24;           // Maximum lifetime in hours for saved vehicles across server restarts, regardless of usage (0 = no time limit)
A3W_vehicleMaxUnusedTime = 14*24;   // Maximum parking time in hours after which unused saved vehicles will be marked for deletion (0 = no time limit)
